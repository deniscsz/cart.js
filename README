...
Cart.js rely on [axios](https://github.com/axios/axios). It accepts 2 params:
| Name | Details |
|-|-|
| `axios` | An axios instance |
| `provider` | Default is `laravel-cart`. Can be overridden with a Provider object (see below) |
...
## Providers
Providers are a way to override the default configuration. Here is the default `laravel-cart` provider:
```javascript
export default {
  urls: {
    createUrl: 'checkout',
